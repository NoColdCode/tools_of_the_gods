	...
	public InteractionResult use(Level world, Player player, InteractionHand hand) {
		...
		if (!world.isClientSide()) {
			...
			if (TraitSystem.hasTrait(stack, TraitSystem.Trait.SILKY_II)) {
				...
				player.sendOverlayMessage(Component.literal("\u00a7aSilk Touch: \u00a7f" + (on ? "ON" : "OFF")));
				toggled = true;
			}
			if (TraitSystem.hasTrait(stack, TraitSystem.Trait.AUTOSMELT_II)) {
				boolean on = TraitSystem.toggleAutosmelt(stack);
				player.sendOverlayMessage(Component.literal("\u00a76Autosmelt: \u00a7f" + (on ? "ON" : "OFF")));
				toggled = true;
			}
			if (TraitSystem.hasTrait(stack, TraitSystem.Trait.BROAD_TOUCH_II)) {
				...
				player.sendOverlayMessage(Component.literal("\u00a75Broad Touch: \u00a7f" + label));
				toggled = true;
			}
			if (toggled) {
		...
		if (!world.isClientSide()) {
			player.sendOverlayMessage(Component.literal("\u00a7bPower Mode: \u00a7f" + POWER_MODE_NAMES[newMode]));
		}
		return InteractionResult.SUCCESS;
	}
