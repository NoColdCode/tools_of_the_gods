...
public final class OrbLootLogic {
	...
	public static void onLivingDrops(TogLivingDropsContext ctx) {
		...
		if (ctx.source().getEntity() instanceof Player player) {
			if (level.getRandom().nextInt(TRAIT_REMOVER_MOB_CHANCE) == 0) {
			...
			if (level.getRandom().nextInt(MOB_DROP_CHANCE) == 0) {
				int tier = getHeldTier(player);
				ItemStack gem = ToolsOfTheGodsOrbItems.getRandomGem(level.getRandom(), Math.max(3, tier + 1));
	...
	public static void onBlockBreak(TogBlockBreakContext ctx) {
		...
		if (level.getRandom().nextInt(BLOCK_DROP_CHANCE) == 0) {
			int tier = getHeldTier(player);
			ItemStack gem = ToolsOfTheGodsOrbItems.getRandomGem(level.getRandom(), Math.max(3, tier));
			level.addFreshEntity(new ItemEntity(level, ctx.pos().getX() + 0.5, ctx.pos().getY() + 0.8, ctx.pos().getZ() + 0.5, gem));
			return;
		}

		if (level.getRandom().nextInt(TRAIT_REMOVER_BLOCK_CHANCE) == 0) {
