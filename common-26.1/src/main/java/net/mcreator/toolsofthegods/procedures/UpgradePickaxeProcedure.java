...
public class UpgradePickaxeProcedure {
	...
	public static boolean tryUpgrade(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack, boolean notifyPlayer) {
		...
		int currentLevel = (int) itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDoubleOr("level", 0.0);
		int currentTier = (int) itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDoubleOr("tier", 0.0);
		boolean needsUpgrade = itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getBooleanOr("needsUpgrade", false);
		...
		boolean hasResources = switch (nextTier) {
			case 1 -> ToolProgressionHelper.usesArmorProgression(toolType)
				? TogEntityInventoryHelper.consume(entity, new ItemStack(Items.IRON_CHAIN), 8)
		...
		if (hasResources) {
			...
			if (world instanceof Level level && !level.isClientSide()) {
				level.playSound(null, BlockPos.containing(x, y, z),
					BuiltInRegistries.SOUND_EVENT.getValue(Identifier.parse("block.anvil.use")),
			...
			if (notifyPlayer && entity instanceof Player player) {
				...
				player.sendSystemMessage(Component.literal("§a§lUPGRADED! §6" + tierLabel + " Tier §aunlocked!"));
			}
			return true;
		}
