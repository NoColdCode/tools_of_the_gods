	...
	private static void applyXpGain(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack stack,
		...
		if (isBoundary(type, newLevel)) {
			...
			if (entity instanceof Player player) {
				player.sendSystemMessage(Component.literal("§6§lLevel " + newLevel + " reached! §eRequires tier upgrade."));
				player.sendSystemMessage(Component.literal("§e"
					+ TierSystem.getUpgradeMaterialMessage(currentTier + 1, usesArmorProgression(type))));
			}
		} else {
			CustomData.update(DataComponents.CUSTOM_DATA, stack, tag -> {
				tag.putDouble(NBT_LEVEL, newLevel);
			...
			if (world instanceof Level level && !level.isClientSide()) {
				level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.getValue(Identifier.parse("entity.player.levelup")), SoundSource.PLAYERS, 0.5f, 1.0f);
			}
			if (entity instanceof Player player) {
				player.sendOverlayMessage(Component.literal("§a§lLevel Up! §7" + newLevel));
			}
		}
	}
