...
public final class GuideBookSpawnLogic {
	...
	private static boolean hasReceivedGuide(Player player) {
		...
		if (TogPlayerData.get(player).getBoolean("togGuideBookGiven")) {
	...
	private static CompoundTag readModData(Player player) {
		...
		if (root.contains(TogModConstants.MODID, Tag.TAG_COMPOUND)) {

			return root.getCompound(TogModConstants.MODID);
