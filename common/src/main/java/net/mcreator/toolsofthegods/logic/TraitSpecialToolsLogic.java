...
public final class TraitSpecialToolsLogic {
	...
	public static void onItemFished(TogItemFishedContext ctx) {
		...
		if (treasureChance > 0.0f && player.level().random.nextFloat() <= treasureChance) {
			ItemStack bonus = switch (player.level().random.nextInt(4)) {
				case 0 -> new ItemStack(Items.EMERALD, 1 + player.level().random.nextInt(2));
				case 1 -> new ItemStack(Items.LAPIS_LAZULI, 2 + player.level().random.nextInt(4));
		...
		if (TraitSystem.hasTrait(rod, TraitSystem.Trait.BOUNTIFUL_I)) {
			for (ItemStack stack : ctx.drops()) {
				if (!stack.isEmpty() && player.level().random.nextFloat() <= 0.10f) {
		...
		if (direct instanceof AbstractArrow) {
			...
			if (!weapon.isEmpty()) {
				...
				if (TraitSystem.hasChanneling(weapon) && player.level() instanceof ServerLevel serverLevel
					...
					if (bolt != null) {
						bolt.moveTo(ctx.entity().getX(), ctx.entity().getY(), ctx.entity().getZ());
