...
public final class WingsFlightLogic {
	...
	public static int getLevel(ItemStack wings) {
		...
		return (int) wings.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("level");
	}
	...
	public static float getResistanceSeconds(ItemStack wings) {
		...
		return switch (getMode(wings)) {
			case CAPE -> 4.0f + level * 0.20f;                 // ~4–8s (tiers 0–1)
			case ELYTRA -> 10.0f + Math.max(0, level - 20) * 0.50f; // ~10–30s
			case ICARUS -> 35.0f + Math.max(0, level - 60) * 1.40f; // ~35–90s
		};
	}
	...
	/** Elytra horizontal speed factor: slower early in elytra tiers; boosted in Wings mode. */
	...
	/** Cape slow-fall + Icarus grounded checks; call every player tick while wings worn. */
	public static void onPlayerTick(Player player) {
		...
		} else if (mode == Mode.ICARUS && airborne && hasStamina(wings)) {
			// Look up: deploy elytra so wings stay open, then climb.
	...
	private static int getAirTicks(ItemStack wings) {
		return wings.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getInt(NBT_AIR_TICKS);
	}
