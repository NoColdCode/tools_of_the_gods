# Smoke-test Forge runClient for each MC version (one at a time).
param(
    [int]$TimeoutSeconds = 240,
    [string[]]$Versions = @('1.18.2', '1.19.2', '1.20.1')
)
...
foreach ($v in $Versions) {
    Write-Host "`n========== forge-$v ==========" -ForegroundColor Cyan

    Write-Host "Building..."
    $buildCode = Invoke-GradleQuiet @(":forge-${v}:build", '--no-daemon', '-PtogLoader=forge', "-PforgeVersions=$v")
    if ($buildCode -ne 0) {
    ...
    $runDir = Join-Path $root "forge\$v\run"
    ...
    $proc = Start-Process -FilePath ".\gradlew.bat" -ArgumentList @(
        ":forge-${v}:runClient", "--no-daemon", "-PtogLoader=forge", "-PforgeVersions=$v"
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        if (Test-Path $logFile) {
            ...
            if ($log -match 'tools_of_the_gods' -and ($log -match 'Loaded \d+ recipes' -or $log -match 'OpenAL initialized' -or $log -match 'Created: \d+x\d+' -or $log -match 'Tools of the Gods .+ loaded on Forge')) {
                $clientOk = $true
