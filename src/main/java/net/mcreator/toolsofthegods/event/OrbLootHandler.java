...
public class OrbLootHandler {
	...
	public static void onLivingDrops(LivingDropsEvent event) {
		...
		if (event.getSource().getEntity() instanceof Player player) {
			if (level.random.nextInt(TRAIT_REMOVER_MOB_CHANCE) == 0) {
				event.getDrops().add(new ItemEntity(level, event.getEntity().getX(), event.getEntity().getY() + 0.5, event.getEntity().getZ(), new ItemStack(ToolsOfTheGodsOrbItems.TRAIT_REMOVER.get())));
				return;
			}
			if (level.random.nextInt(MOB_DROP_CHANCE) == 0) {
				...
				event.getDrops().add(new ItemEntity(level, event.getEntity().getX(), event.getEntity().getY() + 0.5, event.getEntity().getZ(), gem));
			}
		}
	}
	...
	public static void onBlockBreak(BlockEvent.BreakEvent event) {
		...
		if (level.random.nextInt(BLOCK_DROP_CHANCE) == 0) {
			...
			level.addFreshEntity(new ItemEntity(level, event.getPos().getX() + 0.5, event.getPos().getY() + 0.8, event.getPos().getZ() + 0.5, gem));
			return;
		}

		if (level.random.nextInt(TRAIT_REMOVER_BLOCK_CHANCE) == 0) {
			level.addFreshEntity(new ItemEntity(level, event.getPos().getX() + 0.5, event.getPos().getY() + 0.8, event.getPos().getZ() + 0.5, new ItemStack(ToolsOfTheGodsOrbItems.TRAIT_REMOVER.get())));
		}
	}
