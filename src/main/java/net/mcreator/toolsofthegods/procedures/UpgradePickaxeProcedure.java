	...
	public static boolean tryUpgrade(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack, boolean notifyPlayer) {
		...
		if (!needsUpgrade) {
			if (notifyPlayer && entity instanceof Player player) {
				player.displayClientMessage(Component.literal("§cThis tool doesn't need an upgrade yet!"), false);
			}
			return false;
		}
		...
		boolean hasResources = switch (nextTier) {
			case 1 -> toolType == ToolProgressionHelper.ToolType.ARMOR
		...
		if (hasResources) {
			...
			if (notifyPlayer && entity instanceof Player player) {
				player.displayClientMessage(Component.literal("§a§lUPGRADED! §6" + TierSystem.getTierName(nextTier) + " Tier §aunlocked!"), false);
			}
			return true;
		}

		if (notifyPlayer && entity instanceof Player player) {
			player.displayClientMessage(Component.literal("§cInsufficient materials! "
				+ TierSystem.getUpgradeMaterialMessage(nextTier, toolType == ToolProgressionHelper.ToolType.ARMOR)), false);
		}
		return false;
	}
}
