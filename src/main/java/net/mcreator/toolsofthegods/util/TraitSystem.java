...
public class TraitSystem {
	...
	public static List<Trait> getTraits(ItemStack stack) {
		String raw = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getString(NBT_TRAITS);
	...
	public static int getAdditionalSlotCostForNextTrait(ItemStack toolStack, ItemStack ingredientStack) {
		...
		for (Trait owned : getTraits(toolStack)) {
			if (sameFamily(owned, nextTrait)) {
				currentFamilyCost = Math.max(currentFamilyCost, owned.slotCost());
			}
		}
		return Math.max(0, nextTrait.slotCost() - currentFamilyCost);
	}
	...
	public static float getMiningSpeedMultiplier(ItemStack stack) {
		return Math.max(0.05f, 1.0f + getMiningSpeedBonus(stack) + getSynergyMiningSpeedBonus(stack));
	}
	...
	public static List<Component> getTraitTooltip(ItemStack stack) {
		...
		for (Trait trait : traits) {
			lines.add(Component.literal("§d- " + trait.displayName() + " §8(" + trait.slotCost() + " slot" + (trait.slotCost() > 1 ? "s" : "") + ")§7: " + trait.description()));
		}
	...
	public static int getMomentumBlocks(ItemStack stack) {
		return stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getInt(NBT_MOMENTUM_BLOCKS);
	}
	...
	public static long getMomentumLastTime(ItemStack stack) {
		return stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getLong(NBT_MOMENTUM_LAST_TIME);
	}
	...
	public static int getBroadTouchMode(ItemStack stack) {
		return stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getInt(NBT_BROAD_MODE);
	}
	...
	public static boolean isSilkyActive(ItemStack stack) {
		...
		if (hasTrait(stack, Trait.SILKY_II)) {
			...
			return !tag.contains(NBT_SILK_ACTIVE) || tag.getBoolean(NBT_SILK_ACTIVE);
		}
		return false;
	}
	...
	public static boolean toggleSilky(ItemStack stack) {
		var tag = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
		boolean current = !tag.contains(NBT_SILK_ACTIVE) || tag.getBoolean(NBT_SILK_ACTIVE);
	...
	public static boolean isAutosmeltActive(ItemStack stack) {
		...
		if (hasTrait(stack, Trait.AUTOSMELT_II)) {
			var tag = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
			return !tag.contains(NBT_AUTOSMELT_ACTIVE) || tag.getBoolean(NBT_AUTOSMELT_ACTIVE);
		}
		return false;
	}
	...
	public static boolean toggleAutosmelt(ItemStack stack) {
		var tag = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
		boolean current = !tag.contains(NBT_AUTOSMELT_ACTIVE) || tag.getBoolean(NBT_AUTOSMELT_ACTIVE);
		boolean newVal = !current;
