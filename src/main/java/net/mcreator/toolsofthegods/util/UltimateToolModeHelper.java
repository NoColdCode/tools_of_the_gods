...
public final class UltimateToolModeHelper {
	...
	public static UltimateToolMode getMode(ItemStack stack) {
		int ordinal = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getInt(NBT_TOOL_MODE);
	...
	public static boolean isManualLockActive(ItemStack stack) {
		long until = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getLong(NBT_MANUAL_UNTIL);
